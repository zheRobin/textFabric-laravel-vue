...
const showTrial = computed(() => {
    return (new Date(props.planSubscription.trial_ends_at) > new Date(props.planSubscription.ends_at));
})
</script>

<template>
    <div v-if="planSubscription.is_active">
        <div v-if="planSubscription.active_trial && showTrial" class="flex bg-yellow-200 gap-x-6 px-2 py-2.5 justify-center">
