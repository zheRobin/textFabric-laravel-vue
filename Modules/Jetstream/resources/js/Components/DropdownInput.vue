...
<template>
    <div class="relative">
        <button type="button" @click="toggleDropdown" class="inline-flex px-4 py-1.5 items-center w-full justify-between rounded-md border-0 py-1.5 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-tf-blue-500 sm:text-sm sm:leading-6">
        ...
        <div id="options" v-if="state.active" class="absolute z-50 w-full py-2 mt-1 bg-white rounded-lg shadow-xl">
            <button type="button" @click="selectOption(option)" v-for="option in props.options" :value="option" class="text-left w-full block px-4 py-1 text-gray-800 hover:bg-tf-blue-500 hover:text-white">
                {{ option }}
            </button>
        </div>
    </div>
