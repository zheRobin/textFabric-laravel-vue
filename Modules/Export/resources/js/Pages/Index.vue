    ...
    <AppLayout title="Export">
        ...
        <div class="max-w-7xl mx-auto mt-8 mb-8 sm:px-6 lg:px-8">
            <div v-if="generateActive && (anyTeamHasRunningCompilations || progress === 0)" class="flex items-center mb-6 text-sm bg-yellow-200 text-yellow-900 border-l-4 border-yellow-400">
                ...
                <span v-if="anyTeamHasRunningCompilations">{{ $t('There :isSingular :count job in the queue in front of you before we can start your processing.', {'isSingular': (runningCompilations.pending === 1 ? 'is' : 'are') , 'count': runningCompilations.pending.toString()}) }}</span>
