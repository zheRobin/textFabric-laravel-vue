<template>
    <Listbox as="div" v-model="selected">
        <ListboxLabel class="block text-sm font-medium leading-6 text-gray-900">{{$t('Assigned to')}}</ListboxLabel>
        ...
        <span class="flex items-center">
          <span class="ml-3 block truncate">{{ $t('Select') }}</span>
        </span>
          ...
          <ChevronUpDownIcon class="h-5 w-5 text-gray-400" aria-hidden="true" />
            ...
            <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
                <ListboxOptions class="absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                    <ListboxOption as="template" v-for="item in props.option" :key="item.id" :value="item" v-slot="{ active, selected }">
